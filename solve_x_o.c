...
/*   Updated: 2022/03/02 12:57:13 by jdavis           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_width_plus(t_flags *info, unsigned int nb, int checker, int sub, char *str)
{
	int		i;
	char	*temp;
	...
	if (info->_minus)
	{
		ft_apply_hash(info, nb, checker, &temp, &i);
...
char	*ft_solve_o_x(t_flags *info, unsigned int nb)
{
	...
	if (info->_width > (int)ft_strlen(str))
	{
		...
		ft_strdel(&str);
		str = ft_width_plus(info, nb, checker, sub, str);
	}
	else
	{
